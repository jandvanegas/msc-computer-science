    ...
    for cls in hlTercets:  # Loop over class labels
        ...
        for w in h_clsLogProb[cls]:  # Loop over all words
            h_clsLogProb[cls][w] = numpy.log(h_clsLogProb[cls][w]) - numpy.log(
                nWordsCls)  # Compute log N_{cls,w} / N
...
def S2_compute_logLikelihoodMatrix(h_clsLogProb, hWordDict, lTercets,
                                   hCls2Idx=None):
    '''
    Compute the matrix of class-conditional log-likelihoods for each class each tercet in lTercets
...
def compute_classPosteriors(S, logPrior=None):
    ...
    ll = scipy.special.logsumexp(J,
                                 axis=0)  # Compute marginal likelihood log f(x)
...
if __name__ == '__main__':
    ...
    S = S1_compute_logLikelihoodMatrix(S1_model, lTercetsEval,
                                       hCls2Idx=hCls2Idx)
    ...
    lTercetsEval = lInfEval + lParEval
    S = S2_compute_logLikelihoodMatrix(S2_model, S2_wordDict, lTercetsEval,
                                       hCls2Idx=hCls2Idx)
